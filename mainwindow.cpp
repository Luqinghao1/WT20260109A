...
#include "wt_datawidget.h" // [修改] 引用新头文件
...
void MainWindow::init()
{
    ...
    for(int i = 0 ; i<7;i++)
    {
        ...
        // 配置导航按钮图标及对应页面索引
        // 顺序调整为：0:项目, 1:数据, 2:图表, 3:模型, 4:拟合, 5:预测, 6:设置
        switch (i) {
        case 0:
    ...
    // Page 1: 数据编辑器 (使用新类 WT_DataWidget)
    m_DataEditorWidget = new WT_DataWidget(ui->pageHand); // [修改] 构造新类
    ui->verticalLayoutHandle->addWidget(m_DataEditorWidget);
    connect(m_DataEditorWidget, &WT_DataWidget::fileChanged, this, &MainWindow::onFileLoaded); // [修改] 信号来源
    connect(m_DataEditorWidget, &WT_DataWidget::dataChanged, this, &MainWindow::onDataEditorDataChanged); // [修改] 信号来源
